...
param(
    ...
    [string]
    $prjAbrv = "BRS",
    # switch to determine if changes are you be made in prod or non-prod tenant
    [bool]
    $prod = $false
)
###
...
if ([string]::IsNullOrEmpty($org)) {
    if ($prod) { $org = "lbgcloudlive" } else { $org = "lbg-cloudfirst" }
}

# initialise our hash tables
$orgGroups, $prjGroups = @{}
...
if ($prod) {
    ...
    Add-Groups -groups $grpHash -parent "$project/Project Administrators"           -children @("$org/ADO_CNEL_GLO_PlatformSRE")
